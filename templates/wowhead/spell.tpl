...
<div id="main">
	...
	<div id="main-contents" class="main-contents">
		...
		<div class="text">
			{if $option.wp_button==1}<a {if $spell.entry!=null} onmouseover="Tooltip.showAtCursor(event, '<sub><img width=15 src=templates/wowhead/images/wowpedia.ico></sub> {#Wsearch#} {"WoWpedia.org"}', 0, 0, 'q')" onmousemove="Tooltip.cursorUpdate(event)" onmouseout="Tooltip.hide()" href="http://www.wowpedia.org/index.php?title=Special:Search&search={$spell.name}&go=Go"{/if} class="button-red{if $spell.entry==null}-disabled{/if}"><em><b><i>Wowpedia</i></b><span>Wowpedia</span></em></a>{/if}
			...
			{if $spell.entry!=null}<h1>{$spell.name}</h1>{else}
<br><div class="inputbox">
<h1><b>{$spell.name}</b></h1>
<div id="inputbox-error"><br><span class=q10>This spell doesn't exist in our database</span>.</div>
<!--  -->
</div><br>{/if}

			{if $spell.entry!=null}<div id="icon{$spell.entry}-generic" style="float: left"></div>
