...
{/if}
{if $page.profile}
	<link rel="stylesheet" type="text/css" href="templates/wowhead/css/profile.css?{$AOWOW_REVISION}" />
{/if}
	<script src="templates/wowhead/js/fx.js?{$AOWOW_REVISION}" type="text/javascript"></script>
	<script src="templates/wowhead/js/locale_{$language}.js?{$AOWOW_REVISION}" type="text/javascript"></script>
	<script src="templates/wowhead/js/Markup-2.js?{$AOWOW_REVISION}" type="text/javascript"></script>
	...
	{* <script type="text/javascript" src="templates/wowhead/js/filters.js?{$AOWOW_REVISION}"></script>
	 TODO PROFILE
	<script src="templates/wowhead/js/profile_gems.js?{$AOWOW_REVISION}" type="text/javascript"></script>
	<script src="templates/wowhead/js/profile3.js?{$AOWOW_REVISION}" type="text/javascript"></script>
	<script src="templates/wowhead/js/profile.js?{$AOWOW_REVISION}" type="text/javascript"></script>
	<script src="templates/wowhead/js/profile2.js?{$AOWOW_REVISION}" type="text/javascript"></script>
	*}
	
{if $page.Mapper or $page.topic}
	<script src="templates/wowhead/js/Mapper_{$language}.js?{$AOWOW_REVISION}" type="text/javascript"></script>
...
{if $user}g_user = {ldelim} id: {$user.id}, name: '{$user.name|escape:"quotes"}', roles: {$user.roles}, permissions: {$user.perms}, settings: '{$user.settings}'{if $upload}, upload_status: '{$upload}'{/if} {rdelim};{/if}
{if $ss_conf}ss_conf = {$ss_conf};{/if}{if $filtered == 'true'}filtered = '1';{/if}{if $wh_ss.status == 'false' && $wh_ss.content != 'noss'}wh_ss = 'load';{/if}{if $wh_cs.status == 'false'}wh_cs = 'load';{/if}{if $wh_model}wh_model = {$wh_model};{/if}
{if $expansion}expansion = {$expansion};{/if}
	</script>
	<script type="text/javascript" src="https://apis.google.com/js/plusone.js">
  {ldelim}parsetags: 'explicit'{rdelim}
</script>
<script>
</script>
<div id="fb-root"></div>