...
var Markup = {
	...
	tags: {
				...
				if (a && a.noLink) {
					...
					if (a && a.needsRaw) {
						...
						var e = Markup._preText(b._rawText.replace(/(https?:\/\/|www\.)([\/_a-z0-9\%\?#@\-\+~&=;:']|\.[a-z0-9\-])+/gi, function (f) {
							matchUrl = Markup._preText(f.replace(/^www/, "https://www"));
							f = Markup._preText(f);
							var g = d.length;
							d.push([matchUrl, f]);
							return "$L" + g
		...
		blip: {
			...
			allowedClass: MARKUP_CLASS_STAFF,
			toHtml: function (a) {
				var b = "https://blip.tv/play/" + a.unnamed;
				var c = "";
		...
		video: {
			...
			rtrim: true,
			toHtml: function (a) {
				return '<div id="companion-ad" style="width:300px;height:250px;float:right;"></div><embed width="512" height="312" src="https://www.zam.com/shared/fp/flowplayer.commercial-3.1.5.swf" flashvars="config=https://www.zam.com/cgi-bin/video-ad-test.pl%3Fembed%3D1" allowfullscreen="true" allowscriptaccess="always" quality="high" bgcolor="#000000" type="application/x-shockwave-flash" pluginspage="https://www.adobe.com/go/getflashplayer" wmode="opaque"></embed>'
			}
		},
		u: {
			empty: false,
		...
		youtube: {
			...
			allowedClass: MARKUP_CLASS_STAFF,
			toHtml: function (a) {
				var b = "https://www.youtube.com/v/" + a.unnamed + "&fs=1";
				var c = "";
				c += '<object width="480" height="300"' + Markup._addGlobalAttributes(a) + '><param name="movie" value="' + b + '">';
				c += '<param name="allowfullscreen" value="true"></param>';
				c += '<param name="allowscriptaccess" value="always"></param>';
...
Markup.tags.flashtext = {
	...
	toHtml: function (a) {
		...
		var flashvars = { xmlpath:"https://wow.tltgame.ru/aowow/flash/flashtext.xml", classname:a.font, divid:d, text:urlencode(a._contents) };
		var params = { menu:"false", wmode:"transparent", base:".", allowScriptAccess:"true" };
		var h = a.height ? a.height : "100%";
		var w = a.width ? a.width : "100%";
		setTimeout(swfobject.embedSWF.bind(null, "https://wow.tltgame.ru/aowow/flash/flashtext.swf", f, w, h, "9.0.0", false, flashvars, params), 10);
		/*var t = '<div id="' + d + '" style="visibility: visible;"></div>';
		t += '<object style="visibility: visible;" id="' + + '" data="https://wow.tltgame.ru/aowow/flash/flashtext.swf" type="application/x-shockwave-flash" height="' + h + '" width="' + w + '">';
		...
		t += '<param value="xmlpath=https://wow.tltgame.ru/aowow/flash/flashtext.xml&classname=' + a.font + '&divid=' + d + '&text=' + urlencode(a._contents) + '" name="flashvars">';
		t += '</object>';*/
		return [t]
	}
};
...
Markup.tags.trinitycore = {
	...
	toHtml: function (a) {
		...
		var b = "https://code.google.com/p/trinitycore/source/detail?r=" + d;
		var c = d.substr(0, 6);
		return "<a href='" + b + "'>" + c + "</a>"
	}
};
